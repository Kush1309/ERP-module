...
import { getStudentTimetables } from '../../services/studentTimetableApi';
...
export default function StudentTimetablePage() {
    ...
    const fetchTimetables = async () => {
        ...
        setError('');
        try {
            const result = await getStudentTimetables({ ...filters, limit: 100 });
            setTimetables(result?.timetables || []);
        } catch (err) {
            setError(err.response?.data?.message || 'Failed to load student timetable');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="mx-auto w-full max-w-6xl pb-10">
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 className="font-display text-3xl font-semibold text-ink-900">My Class Timetable</h1>
